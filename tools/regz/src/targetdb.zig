...
pub fn load_into_db(io: std.Io, db: *Database, path: []const u8, device: ?[]const u8) !void {
    ...
    var modules: std.StringHashMapUnmanaged(ModuleEntry) = .empty;
    defer {
        var key_it = modules.keyIterator();
        while (key_it.next()) |key| {
            db.gpa.free(key.*);
        }
        modules.deinit(db.gpa);
    }
...
fn load_device(
    ...
    filename: []const u8,
    modules: *std.StringHashMapUnmanaged(ModuleEntry),
) !void {
...
fn load_instance(
    ...
    const module_entry = if (modules.get(href)) |module_entry| blk: {
        ...
        try modules.put(db.gpa, href_copy, new_module_entry);
