...
pub fn main() !void {
    ...
    const temp_sensor = try TMP117.init(i2c_device.i2c_device(), @fromBackingInt(@intCast(0x48)));
    ...
    std.log.info("Writing config", .{});
    try i2c0.writev_blocking(@fromBackingInt(@intCast(temp_sensor_address)), &.{&cbuf}, null);

    std.log.info("Reading temp", .{});
    for (0..5) |_| {
        const write_buf = [_]u8{0}; // Read temperature register
        try i2c0.writev_then_readv_blocking(@fromBackingInt(@intCast(temp_sensor_address)), &.{&write_buf}, &.{&temp_buf}, null);
    ...
    std.log.info("Writing config", .{});
    try i2c0dma.write_blocking(@fromBackingInt(@intCast(temp_sensor_address)), &cbuf, null);

    std.log.info("Reading temp", .{});
    for (0..5) |_| {
        const write_buf = [_]u8{0}; // Read temperature register
        try i2c0dma.write_then_read_blocking(@fromBackingInt(@intCast(temp_sensor_address)), &write_buf, &temp_buf, null);
        const temp_u = std.mem.readInt(u16, &temp_buf, .big);
        const temp = @as(f32, @floatFromInt(temp_u)) * 7.8125E-3;
        std.log.info("Temp: {d:0.2}°C", .{temp});
        sleep_ms(500);
