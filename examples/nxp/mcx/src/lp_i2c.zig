...
pub fn main() !void {
    ...
    try i2c_device.write(@fromBackingInt(0x10), data);

    // and read
    var buffer: [4]u8 = undefined;
    _ = try i2c_device.read(@fromBackingInt(0x10), &buffer);

    // or do both
    try i2c_device.write_then_read(@fromBackingInt(0x10), data, &buffer);
}
