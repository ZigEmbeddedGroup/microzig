...
pub fn main() !void {
    ...
    std.log.info("It took {d} µs to replace {d} allocations.", .{ end_time.diff(start_time).to_us(), replace_count });
    ...
    for (0..100) |_| {
        ...
        // allocs[idx] = try allocator.alignedAlloc(u8, .@"32", rand.intRangeAtMost(usize, 1, 256)); // Zig 0.15.0
