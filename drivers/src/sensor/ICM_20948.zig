...
pub const Options = struct {
    /// Controls which log messages the driver compiles in.
    /// Works like `std_options.log_level`: messages at this severity or
    /// higher are included. Set to `null` to compile out all logging.
    /// Default `.err` includes only error messages.
    log_level: ?std.log.Level = .err,
};

pub fn ICM_20948(comptime options: Options) type {
    const log = struct {
        const builtin = @import("builtin");
        fn debug(comptime format: []const u8, args: anytype) void {
            if (builtin.is_test) return;
            const level = options.log_level orelse return;
            if (@backingInt(level) > @backingInt(std.log.Level.debug)) return;
        ...
        fn err(comptime format: []const u8, args: anytype) void {
            if (builtin.is_test) return;
            const level = options.log_level orelse return;
            if (@backingInt(level) > @backingInt(std.log.Level.err)) return;
    ...
    return struct {
        ...
        const AccelDataUnscaled = packed struct {
            x: i16 = 0,
            y: i16 = 0,
            z: i16 = 0,
        };

        pub fn get_accel_data_unscaled(self: *Self) Error!AccelDataUnscaled {
            var raw_data: AccelDataUnscaled = .{};
        ...
        pub fn get_accel_data(self: *Self) Error!AccelData {
        ...
        const GyroDataUnscaled = packed struct {
            x: i16 = 0,
            y: i16 = 0,
            z: i16 = 0,
        };

        pub fn get_gyro_data_unscaled(self: *Self) Error!GyroDataUnscaled {
            var raw_data: GyroDataUnscaled = .{};
        ...
        pub fn get_gyro_data(self: *Self) Error!GyroData {
        ...
        pub fn get_accel_gyro_data(self: *Self) Error!SixDofData {
            var raw_data = packed struct {
                accel: AccelDataUnscaled = .{},
                gyro: GyroDataUnscaled = .{},
                temp: i16 = 0,
            }{};
        ...
        const NineDofData = struct { accel: AccelData, gyro: GyroData, temp: f32, mag: MagData };
        ...
        const MagDataUnscaled = packed struct {
            status1: u8 = 0,
            x: i16 = 0,
            y: i16 = 0,
            z: i16 = 0,
        ...
        pub fn get_mag_data_unscaled(self: *Self) Error!MagDataUnscaled {
            try self.mag_set_sensor_read();
            var raw_data: MagDataUnscaled = .{};
        ...
        // Unit: µT
        const MagData = struct {
            x: f32 = 0,
            y: f32 = 0,
            z: f32 = 0,
        };

        pub fn get_mag_data(self: *Self) Error!MagData {
...
// Testing
const Test_I2C_Device = mdf.base.I2C_Device.TestDevice;
const TestTime = mdf.base.ClockDevice.TestDevice;

test "set_bank" {
    var ttd = TestTime.init();
    var d = Test_I2C_Device.init(null, true);
    defer d.deinit();
