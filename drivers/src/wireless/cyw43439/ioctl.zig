...
const SDP_Header = extern struct {
    len: u16,
    not_len: u16,
    ...
    pub fn channel(sdp: SDP_Header) Chan {
        return sdp.cf.chan;
    }

    pub fn validate(sdp: SDP_Header, n: u16) !void {
        if (sdp.len != n) {
...
const CDC_Header = extern struct {
    ...
    pub fn status_ok(self: CDC_Header) bool {
...
const BDC_Header = extern struct {
    ...
    fn padding(self: BDC_Header) usize {
...
// if chan == .event
//  - 4 bytes bdc header
//  - xx bdc padding bytes (defined in BDC_Header.offset)
...
pub const Response = struct {
    ...
    pub fn data(self: Self) []const u8 {
        ...
        const tail = self.sdp.len - @sizeOf(SDP_Header);
        if (head > tail) {
            return &.{};
        }
...
pub fn request(
    ...
    const header_len: u16 = @sizeOf(SDP_Header) + @sizeOf(CDC_Header);
    const txlen: u16 = header_len + payload_len;

    const sdp: SDP_Header = .{
        .len = txlen,
        .not_len = ~txlen,
        .seq = tx_sequence,
        .cf = .{
...
pub const TxHeader = extern struct {
    ...
    sdp: SDP_Header = mem.zeroes(SDP_Header),
    _padding: u16 = 0,
    bdc: BDC_Header = mem.zeroes(BDC_Header),
...
comptime {
    assert(@sizeOf(SDP_Header) == 12);
    assert(@sizeOf(BDC_Header) == 4);
    assert(@sizeOf(CDC_Header) == 16);
