...
pub const CYW43_Bus = struct {
    ...
    fn writen(self: *Self, func: FuncType, addr: u17, value: u32, len: u11) void {
        const cmd = Cmd{ .cmd = .write, .incr = .incremental, .func = func, .addr = addr, .len = len };
    ...
    pub fn bp_read(self: *Self, addr: u32, data: []u8) void {
        ...
        while (remaining_data.len > 0) {
            ...
            const cmd = Cmd{ .cmd = .read, .incr = .incremental, .func = .backplane, .addr = @truncate(window_offs), .len = @truncate(len) };
    ...
    pub fn bp_write(self: *Self, addr: u32, data: []const u8) void {
        ...
        while (remaining_data.len > 0) {
            ...
            const cmd = Cmd{ .cmd = .write, .incr = .incremental, .func = .backplane, .addr = @truncate(window_offs), .len = @truncate(len) };
            buf[0] = @bitCast(cmd);
    ...
    fn read32_swapped(self: *Self, func: FuncType, addr: u17) u32 {
        const cmd = Cmd{ .cmd = .read, .incr = .incremental, .func = func, .addr = addr, .len = 4 };
    ...
    fn write32_swapped(self: *Self, func: FuncType, addr: u17, value: u32) void {
        const cmd = Cmd{ .cmd = .write, .incr = .incremental, .func = func, .addr = addr, .len = 4 };
