...
//!
//! ClockDevices can be used to track time & sleep
//!

const std = @import("std");
const mdf = @import("../root.zig");

const ClockDevice = @This();
...
pub fn is_reached(td: ClockDevice, time: mdf.time.Absolute) bool {
    const now = td.get_time_since_boot();
    return time.is_reached_by(now);
}

pub fn make_timeout(td: ClockDevice, timeout: mdf.time.Duration) Timeout {
    return .{
        .clock = td,
...
pub fn sleep_ms(td: ClockDevice, time_ms: u32) void {
    td.sleep_us(time_ms * 1000);
}

pub fn sleep_us(td: ClockDevice, time_us: u64) void {
...
/// VTable methods
pub fn get_time_since_boot(td: ClockDevice) mdf.time.Absolute {
...
pub const TestDevice = struct {
    ...
    pub fn init() TestDevice {
        return TestDevice{};
    }

    pub fn elapse_time(dev: *TestDevice, time_us: u64) void {
        dev.time += time_us;
    }

    pub fn set_time(dev: *TestDevice, time_us: u64) void {
        dev.time = time_us;
    }

    pub fn get_total_sleep_time(dev: *TestDevice) u64 {
        return dev.total_sleep_time;
    }

    pub fn reset_sleep_time(dev: *TestDevice) void {
        dev.total_sleep_time = 0;
    }

    pub fn clock_device(dev: *TestDevice) ClockDevice {
        return ClockDevice{
            .ptr = dev,
            .vtable = &vtable,
        };
    ...
    const vtable = VTable{
        .get_time_since_boot = TestDevice.get_time_since_boot_fn,
        .sleep = TestDevice.sleep_fn,
    };
};

test TestDevice {
    var ttd = TestDevice.init();
