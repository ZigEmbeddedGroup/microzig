...
pub const Address = enum(u7) {
    ...
    pub fn format(addr: Address, fmt: []const u8, options: std.fmt.FormatOptions, writer: anytype) !void {
        _ = fmt;
        _ = options;
        try writer.print("I2C(0x{X:0>2})", .{@backingInt(addr)});
    }
};
...
pub const TestDevice = struct {
    ...
    pub fn i2c_device(td: *TestDevice) I2C_Device {
        return I2C_Device{
            .ptr = td,
            .vtable = &vtable,
        };
    ...
    fn writev_then_readv(ctx: *anyopaque, address: Address, write_chunks: []const []const u8, read_chunks: []const []u8) InterfaceError!void {
        try TestDevice.writev(ctx, address, write_chunks);
        _ = try TestDevice.readv(ctx, address, read_chunks);
    }
