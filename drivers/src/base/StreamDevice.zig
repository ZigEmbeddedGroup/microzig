...
/// Writes some `bytes` to the device and returns the number of bytes written.
pub fn writev(sd: StreamDevice, bytes_vec: []const []const u8) WriteError!usize {
...
/// Reads some `bytes` to the device and returns the number of bytes read.
pub fn readv(sd: StreamDevice, bytes_vec: [][]u8) ReadError!usize {
    const readv_fn = sd.vtable.readv_fn orelse return error.Unsupported;
    return readv_fn(sd.ptr, bytes_vec);
}

pub fn writer(self: StreamDevice, buf: []u8) Writer {
    return .init(self, buf);
}

pub fn reader(self: StreamDevice, buf: []u8) Reader {
    return .init(self, buf);
}
...
pub const VTable = struct {
    ...
    readv_fn: ?*const fn (*anyopaque, datagram: [][]u8) ReadError!usize,
};
...
pub const Writer = struct {
    ...
    pub fn init(dev: StreamDevice, buf: []u8) Writer {
        return Writer{
            .dev = dev,
            .interface = .{
                .buffer = buf,
...
pub const Reader = struct {
    ...
    pub fn init(dev: StreamDevice, buf: []u8) Reader {
        return Reader{
            .dev = dev,
            .interface = .{
                .buffer = buf,
...
pub const TestDevice = struct {
    ...
    pub fn init(input: ?[]const u8, write_enabled: bool) TestDevice {
        return TestDevice{
            .connected = false,

            .input = if (input) |bytes| .fixed(bytes) else null,
    ...
    pub fn stream_device(td: *TestDevice) StreamDevice {
        return StreamDevice{
            .ptr = td,
            .vtable = &vtable,
        };
    ...
    pub fn expect_all_input_consumed(td: TestDevice) !void {
        // No input is totally fine and counts as "all input consumed"
        const input = td.input orelse return;

        try std.testing.expectEqualSlices(u8, "", input.buffer[input.seek..]);
    }

    /// Expects that all sent data is `expected`.
    pub fn expect_sent(td: TestDevice, expected: []const u8) !void {
        if (td.output) |output| {
    ...
    fn readv(ctx: *anyopaque, bytes_vec: [][]u8) ReadError!usize {
        const td: *TestDevice = @ptrCast(@alignCast(ctx));
        if (!td.connected) {
            return error.NotConnected;
        }

        const data_reader = if (td.input) |*r| r else return error.Unsupported;
        return data_reader.readVec(bytes_vec) catch error.IoError;
    }
};
...
test TestDevice {
    ...
    var td = TestDevice.init(input_text, true);
    defer td.deinit();
