...
pub fn init(datagram_device: anytype, frequency: u32) !PCA9685(.{ .DatagramDevice = @TypeOf(datagram_device) }) {
    const Type = PCA9685(.{
...
pub fn PCA9685(comptime config: PCA9685_Config) type {
    return struct {
        ...
        fn init(datagram_device: config.DatagramDevice, frequency: u32) !Self {
            const self = Self{
                .dd = datagram_device,
            };
