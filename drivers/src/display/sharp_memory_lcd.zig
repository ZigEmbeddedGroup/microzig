...
pub const Config = struct {
    ...
    /// Datagram device type (typically SPI_DatagramDevice)
...
pub fn SharpMemory_LCD(comptime config: Config) type {
    ...
    return struct {
        ...
        pub const Framebuffer = FramebufferGeneric(config.width, config.height);
...
fn FramebufferGeneric(width: comptime_int, height: comptime_int) type {
    return struct {
        const Self = @This();
...
test "framebuffer LSB-first pixel ordering" {
    const Fb = FramebufferGeneric(8, 1);
...
test "framebuffer line-major layout" {
    const Fb = FramebufferGeneric(16, 2);
    var fb = Fb.init(.white);
