...
pub fn init(comptime mode: DriverMode, device: anytype, digital_io: anytype) !SSD1306_Generic(.{
    .mode = mode,
    .DatagramDevice = @TypeOf(device),
...
pub fn SSD1306_Generic(comptime options: SSD1306_Options) type {
    ...
    return struct {
        ...
        fn init_without_io(dev: DatagramDevice) !Self {
            var self = Self{
                .dd = dev,
                .dc_pin = {},
                .mode = {},
        ...
        fn init_with_mode(mode: DriverInitMode) !Self {
            var self = Self{
                .dd = switch (mode) {
                    .i2c => |opt| opt.device,
