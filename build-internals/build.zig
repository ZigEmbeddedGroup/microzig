...
pub const Target = struct {
    ...
    linker_script: LinkerScript = .{ .generate = .{} },
...
pub const LinkerScript = union(enum) {
    pub const GenerateOptions = struct {
        pub const Mode = union(enum) {
            ...
            memory_regions_and_sections: struct {
                ...
                /// Where should `microzig_time_critical` go?
                time_critical_location: enum {
                    /// Place `microzig_time_critical` in the first region tagged as ram if it is
                    /// executable or else in the first region tagged as flash.
                    auto,
                    /// Place `microzig_time_critical` in the first region tagged as flash.
                    flash,
                    /// Place `microzig_time_critical` in the first region tagged as ram. Panics if
                    /// the region is not executable.
                    ram,
                    /// Don't place `microzig_time_critical` in the auto-generated sections.
                    omit,
                } = .auto,
            },
        };
        ...
        mode: Mode = .{ .memory_regions_and_sections = .{} },
    ...
    custom: LazyPath,
    /// Auto-generate the linker script (or parts of it).
