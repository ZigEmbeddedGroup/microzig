...
/// This function returns an allocator class that uses the supplied Mutex type for synchronization.
///
/// Parameters:
/// - `Mutex`: This must be a type that implements the `Mutex` interface.
///            Passing `null` will use the `microzig.interrupt.Mutex`.
///
pub fn Allocator(Mutex: ?type) type {

    const MutexType = if (Mutex) |T| T else microzig.interrupt.Mutex;
    ...
    const Alloc = struct {
        ...
        pub fn setupWithBuffer(buffer: []u8) Alloc {
            ...
            // std.log.debug("Allocator: {x}: Initial chunk 0x{x:08}", .{ @intFromPtr(&self), @intFromPtr(init_chunk) });
        ...
        fn do_alloc(ptr: *anyopaque, len: usize, alignment: Alignment, _: usize) ?[*]u8 {
            ...
            //std.log.debug("\n *** Allocating {} bytes of memory with alignment {d} needed {}", .{ len, alignment.toByteUnits(), needed });
        ...
        pub const Chunk = extern struct {
            ...
            pub fn combine_and_free(self: *Chunk, alloc: *Alloc) void {
                ...
                // std.log.debug("Allocator: {x}: Combining and freeing chunk 0x{x:08}", .{ @intFromPtr(alloc), @intFromPtr(chunk) });
                ...
                // std.log.debug("Adding chunk 0x{x:08} to free list {d}", .{ @intFromPtr(chunk), free_index });
        ...
        pub fn dbg_integrity_check(self: *Alloc) bool {
            ...
            if (!valid) {
                self.dbg_log_free_chains();
                self.dbg_log_chunk_list();
            }
