...
pub const interrupt = struct {
    ...
    const priority_bits = microzig.chip.properties.interrupt_priority_bits orelse 2;
...
pub const peripherals = struct {
    ...
    pub const dcb: (if (@hasDecl(core, "DebugControlBlock"))
        *volatile core.DebugControlBlock
    else
        *volatile anyopaque) = @ptrFromInt(dcb_base);

    pub const dwt: (if (@hasDecl(core, "DataWatchpointAndTrace"))
        *volatile core.DataWatchpointAndTrace
    else
        *volatile anyopaque) = @ptrFromInt(dwt_base);
