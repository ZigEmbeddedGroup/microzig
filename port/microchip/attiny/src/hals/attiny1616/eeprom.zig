...
pub const Address = enum(u8) {
    ...
    pub fn fromInt(value: u8) Address {
...
pub fn writeByte(address: Address, value: u8) void {
    busyWait();
    regs.mem8(data_start + @as(u16, @intFromEnum(address))).* = value;
    executeCommand(command_page_erase_write);
}

pub fn updateByte(address: Address, value: u8) void {
    if (readByte(address) != value) writeByte(address, value);
}

pub fn readSlice(comptime len: usize, start: Address) [len]u8 {
    var out: [len]u8 = undefined;
    for (&out, 0..) |*byte, offset| {
        byte.* = readByte(@enumFromInt(@intFromEnum(start) + offset));
    }
    return out;
}
...
fn executeCommand(command: u8) void {
