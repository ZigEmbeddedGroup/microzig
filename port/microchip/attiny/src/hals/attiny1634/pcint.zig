...
pub inline fn enableGroup(group: Group) void {
    regs.setBits(regs.GIMSK, switch (group) {
        .pcint0_7 => 1 << 4,
        .pcint8_11 => 1 << 5,
        .pcint12_17 => 1 << 6,
    });
...
pub inline fn enablePin(group: Group, bit: u3) void {
    ...
    regs.setBits(maskRegister(group), regs.bit(bit));
}

fn maskRegister(group: Group) u16 {
    return switch (group) {
