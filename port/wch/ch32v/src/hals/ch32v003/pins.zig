        ...
        if (@field(config, port_field.name)) |port_config| {
            for (@typeInfo(Port.Configuration).@"struct".field_names) |field_name| {
                if (@field(port_config, field_name)) |pin_config| {
                    const name: []const u8 = pin_config.name orelse get_tag_name_by_index(PortName, @intFromEnum(@field(Port, port_field.name))) ++ @tagName(@field(Pin, field_name))[3..];
                    const typ: type = GPIO(@intFromEnum(@field(Port, port_field.name)), @intFromEnum(@field(Pin, field_name)), pin_config.mode orelse .{ .input = .{.floating} });
...
pub const PortName = enum {
    PA,
    // NOTE: PB is requered to get by index
    PB,
    PC,
    PD,
};
        ...
        inline for (@typeInfo(GlobalConfiguration).@"struct".fields) |port_field| {
            if (@field(config, port_field.name)) |port_config| {
                ...
                inline for (@typeInfo(Port.Configuration).@"struct".field_names) |field_name| {
                    ...
                    if (@field(port_config, field_name)) |pin_config| {
                        var pin = gpio.Pin.init(@intFromEnum(@field(Port, port_field.name)), @intFromEnum(@field(Pin, field_name)));
