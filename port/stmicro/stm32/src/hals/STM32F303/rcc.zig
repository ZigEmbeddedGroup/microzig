...
fn apply_flash_flash() void {
    const latency: LATENCY = @fromBackingInt(@backingInt(current_clocks.config.FLatency));
...
fn apply_pll() void {
    ...
    const source: PLLSRC = @fromBackingInt(@backingInt(current_clocks.config.PLLSourceVirtual));
    const mul: PLLMUL = @fromBackingInt(@backingInt(current_clocks.config.PLLMUL));

    //only on STM32F303E clocktree for DIE446
    comptime if (Tree.check_MCU("DIE446")) {
        const divider: PREDIV = @fromBackingInt(@backingInt(current_clocks.config.PLLDivider));
...
fn apply_prescaler() void {
    const apb1: PPRE = @fromBackingInt(@backingInt(current_clocks.config.APB1CLKDivider));
    const apb2: PPRE = @fromBackingInt(@backingInt(current_clocks.config.APB2CLKDivider));
    const ahb: HPRE = @fromBackingInt(@backingInt(current_clocks.config.AHBCLKDivider));
    const adc12: ADCPRES = @fromBackingInt(@backingInt(current_clocks.config.ADC12PRES));
    const adc34: ADCPRES = @fromBackingInt(@backingInt(current_clocks.config.ADC34PRES));
    const usbprescal: USBPRE = @fromBackingInt(@backingInt(current_clocks.config.PRESCALERUSB));
...
pub fn select_clock() void {
    const sys_clk: SW = @fromBackingInt(@backingInt(current_clocks.config.SYSCLKSourceVirtual));
    const i2s_clk: ISSRC = @fromBackingInt(@backingInt(current_clocks.config.I2SClockSource));
    const i2c1_clk: ICSW = @fromBackingInt(@backingInt(current_clocks.config.I2c1ClockSelection));
    const i2c2_clk: ICSW = @fromBackingInt(@backingInt(current_clocks.config.I2c2ClockSelection));

    const usart1_clk: USART1SW = @fromBackingInt(@backingInt(current_clocks.config.Usart1ClockSelection));
    const usart2_clk: USARTSW = @fromBackingInt(@backingInt(current_clocks.config.Usart2ClockSelection));
    const usart3_clk: USARTSW = @fromBackingInt(@backingInt(current_clocks.config.Usart3ClockSelection));
    const uart4_clk: USARTSW = @fromBackingInt(@backingInt(current_clocks.config.Uart4ClockSelection));
    const uart5_clk: USARTSW = @fromBackingInt(@backingInt(current_clocks.config.Uart5ClockSelection));
