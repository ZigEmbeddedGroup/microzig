...
pub fn Uart(comptime index: UartNum) type {
    ...
    return struct {
        ...
        fn writer_fn(self: *const Self, buffer: []const u8) error{}!usize {
            for (buffer) |byte| {
                self.tx(byte);
            }
            return buffer.len;
...
pub fn UartWriter(comptime index: UartNum) type {
    return struct {
        uart: *const Uart(index),
        interface: std.Io.Writer,

        pub fn init(uart: *const Uart(index), buffer: []u8) UartWriter(index) {
            return .{
                .uart = uart,
                .interface = .{
                    .vtable = &.{
...
///    .logFn = hal.uart.log,
///};
pub fn init_logger(uart: *const Uart(.UART1)) void {
