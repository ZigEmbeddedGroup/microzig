...
pub fn set_line_edge(line: u5, edge: TriggerEdge) void {
    ...
    switch (edge) {
        .None => {
            EXTI.@"RTSR[0]".clear_raw(mask);
            EXTI.@"FTSR[0]".clear_raw(mask);
        },
        .rising => {
            EXTI.@"RTSR[0]".set_raw(mask);
            EXTI.@"FTSR[0]".clear_raw(mask);
        },
        .falling => {
            EXTI.@"RTSR[0]".clear_raw(mask);
            EXTI.@"FTSR[0]".set_raw(mask);
        },
        .both => {
            EXTI.@"RTSR[0]".set_raw(mask);
            EXTI.@"FTSR[0]".set_raw(mask);
...
pub fn set_event(line: u5, enable: bool) void {
    const mask = @as(u32, 1) << line;
    if (enable)
        EXTI.@"EMR[0]".set_raw(mask)
    else
        EXTI.@"EMR[0]".clear_raw(mask);
}

pub fn set_interrupt(line: u5, enable: bool) void {
    const mask = @as(u32, 1) << line;
    if (enable)
        EXTI.@"IMR[0]".set_raw(mask)
    else
        EXTI.@"IMR[0]".clear_raw(mask);
}
...
pub inline fn software_trigger(line: u5) void {
    EXTI.SWIER.set_raw(@as(u32, 1) << line);
}
