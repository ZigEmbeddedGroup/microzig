...
pub fn secure_enable() void {
    ...
    rcc.BDCR.write_raw(0);
    rcc.CFGR.write_raw(0);
...
pub fn reset_clock(comptime peri: Peripherals) void {
    ...
    @field(rcc, rcc_register_name).write_raw(0);
}
...
pub fn enable_all_clocks() void {
    //enable all clocks
    rcc.AHBENR.write_raw(std.math.maxInt(u32));
    rcc.APB1ENR.write_raw(std.math.maxInt(u32));
    rcc.APB2ENR.write_raw(std.math.maxInt(u32));
}

pub fn disable_all_clocks() void {
    //disable all clocks
    rcc.AHBENR.write_raw(0);
    rcc.APB1ENR.write_raw(0);
    rcc.APB2ENR.write_raw(0);
}
...
pub fn reset_bus(bus: Bus) void {
    ...
    switch (bus) {
        .APB1 => {
            rcc.APB1RSTR.write_raw(std.math.maxInt(u32));
            rcc.APB1RSTR.write_raw(0);
        },
        .APB2 => {
            rcc.APB2RSTR.write_raw(std.math.maxInt(u32));
            rcc.APB2RSTR.write_raw(0);
        },
    }
}
