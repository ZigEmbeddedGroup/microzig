...
fn timer_get_time_since_boot() time.Absolute {
    const tim = tim_ctx orelse return @fromBackingInt(@intCast(0));
    ...
    return @fromBackingInt(@intCast(ticks));
}
...
pub fn get_time_since_boot() time.Absolute {
    const callback = get_time orelse return @fromBackingInt(@intCast(0));
    return callback();
}
