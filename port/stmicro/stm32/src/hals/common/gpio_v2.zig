...
pub const Pin = enum(usize) {
    ...
    pub inline fn set_bias(gpio: Pin, bias: PUPDR) void {
        ...
        port.PUPDR.write_raw((port.PUPDR.read_raw() & ~modMask) | @as(u32, @intFromEnum(bias)) << (pin << 1));
    }
    ...
    pub inline fn set_speed(gpio: Pin, speed: OSPEEDR) void {
        ...
        port.OSPEEDR.write_raw((port.OSPEEDR.read_raw() & ~modMask) | @as(u32, @intFromEnum(speed)) << (pin << 1));
    }
    ...
    pub inline fn set_moder(gpio: Pin, moder: MODER) void {
        ...
        port.MODER.write_raw((port.MODER.read_raw() & ~modMask) | @as(u32, @intFromEnum(moder)) << (pin << 1));
    }
    ...
    pub inline fn set_output_type(gpio: Pin, otype: OT) void {
        ...
        port.OTYPER.write_raw((port.OTYPER.read_raw() & ~gpio.mask()) | @as(u32, @intFromEnum(otype)) << pin);
    }
    ...
    pub inline fn set_alternate_function(gpio: Pin, afr: AF) void {
        ...
        register.write_raw((register.read_raw() & ~afrMask) | @as(u32, @intFromEnum(afr)) << ((pin % 8) << 2));
    }
