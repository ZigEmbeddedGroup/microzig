...
pub fn ClockTree(comptime mcu_data: std.StaticStringMap(void)) type {
    return struct {
        ...
        pub const Flag_Output = struct {
            ...
            UCPD1Enable: bool = false,
            SPI1Enable: bool = false,
            SPI3Enable: bool = false,
            SPI2Enable: bool = false,
            USBPHYEnableHSE: bool = false,
            USBPHYEnable: bool = false,
            EnableDSI: bool = false,
            USBPHYEnablePll: bool = false,
        ...
        pub fn get_clocks(config: Config) anyerror!Tree_Output {
            ...
            if (check_MCU("HSPI1_Exist")) {
                if (check_ref(@TypeOf(HSPI1EnableValue), HSPI1EnableValue, .true, .@"=")) {
                    const HSPIMult_clk_value = HSPICLockSelectionValue orelse return comptime_fail_or_error(error.InvalidClockValue,
                        \\Error on Clock {s} | expr: {s} diagnostic: {s}
                        \\Clock is active but the reference value {s} is null
                        \\note: check the flags and configurations associated with this clock
                        \\note2: actually I'm not sure if null values and clock nodes should be treated as errors.
            ...
            if (check_MCU("I2C6_Exist")) {
                if (check_ref(@TypeOf(I2C6EnableValue), I2C6EnableValue, .true, .@"=")) {
                    const I2C6Mult_clk_value = I2C6CLockSelectionValue orelse return comptime_fail_or_error(error.InvalidClockValue,
                        \\Error on Clock {s} | expr: {s} diagnostic: {s}
                        \\Clock is active but the reference value {s} is null
                        \\note: check the flags and configurations associated with this clock
                        \\note2: actually I'm not sure if null values and clock nodes should be treated as errors.
            ...
            if (check_ref(@TypeOf(USBPHYEnableHSEValue), USBPHYEnableHSEValue, .true, .@"=")) {
                const HSEDiv2_clk_value = HSEDiv2Value orelse return comptime_fail_or_error(error.InvalidClockValue,
                    \\Error on Clock {s} | expr: {s} diagnostic: {s}
                    \\Clock is active but the reference value {s} is null
                    \\note: check the flags and configurations associated with this clock
                    \\note2: actually I'm not sure if null values and clock nodes should be treated as errors.
            ...
            if (check_ref(@TypeOf(USBPHYEnableValue), USBPHYEnableValue, .true, .@"=")) {
                const PLL1PDiv2_clk_value = PLL1PDiv2Value orelse return comptime_fail_or_error(error.InvalidClockValue,
                    \\Error on Clock {s} | expr: {s} diagnostic: {s}
                    \\Clock is active but the reference value {s} is null
                    \\note: check the flags and configurations associated with this clock
                    \\note2: actually I'm not sure if null values and clock nodes should be treated as errors.
            ...
            if (check_ref(@TypeOf(USBPHYEnableValue), USBPHYEnableValue, .true, .@"=")) {
                const USBPHYMult_clk_value = USBPHYCLockSelectionValue orelse return comptime_fail_or_error(error.InvalidClockValue,
                    \\Error on Clock {s} | expr: {s} diagnostic: {s}
                    \\Clock is active but the reference value {s} is null
                    \\note: check the flags and configurations associated with this clock
                    \\note2: actually I'm not sure if null values and clock nodes should be treated as errors.
            ...
            //POST CLOCK REF HSPI1Freq_Value VALUE
            _ = blk: {
                if (scale4 or check_MCU("S_LPBAM_CONF")) {
                    HSPIoutput.limit = .{
                        .min = null,
            ...
            _ = blk: {
                ...
                I2C5output.value = 4000000;
                break :blk null;
            };

            //POST CLOCK REF I2C6Freq_Value VALUE
            _ = blk: {
                if (scale4 or check_MCU("S_LPBAM_CONF")) {
                    I2C6output.limit = .{
                        .min = null,
            ...
            const FLatencyValue: ?FLatencyList = blk: {
                ...
                } else if ((scale4 and ((check_ref(?f32, AHBOutput.get_as_ref(), 25000000, .@"<")) or (check_ref(?f32, AHBOutput.get_as_ref(), 25000000, .@"="))))) {
            ...
            const PWR_Regulator_Voltage_ScaleValue: ?PWR_Regulator_Voltage_ScaleList = blk: {
                if (check_MCU("S_LPBAM_CONF")) {
                ...
                } else if ((check_ref(?f32, SysCLKOutput.get_as_ref(), 160000000, .@"<")) or (check_ref(?f32, SysCLKOutput.get_as_ref(), 160000000, .@"="))) {
                    scale1 = true;
                    ...
                    break :blk item;
                } else if ((check_ref(?f32, SysCLKOutput.get_as_ref(), 160000000, .@">"))) {
                    scale1 = true;
                    const item: PWR_Regulator_Voltage_ScaleList = .PWR_REGULATOR_VOLTAGE_SCALE1;
                    const conf_item = config.extra.PWR_Regulator_Voltage_Scale;
                    if (conf_item) |i| {
            ...
            ref_out.flags.OTG_HS_Used = config.flags.OTG_HS_Used;
            ...
            ref_out.flags.HSPI1_Used = config.flags.HSPI1_Used;
            ref_out.flags.LTDC_Used = config.flags.LTDC_Used;
            ref_out.flags.DSIHOST_Used = config.flags.DSIHOST_Used;
            ...
            ref_out.flags.HSPI1Enable = check_ref(?HSPI1EnableList, HSPI1EnableValue, .true, .@"=");
            ...
            ref_out.flags.I2C6Enable = check_ref(?I2C6EnableList, I2C6EnableValue, .true, .@"=");
            ...
            ref_out.flags.UCPD1Enable = check_ref(?UCPD1EnableList, UCPD1EnableValue, .true, .@"=");
            ...
            ref_out.flags.USBPHYEnableHSE = check_ref(?USBPHYEnableHSEList, USBPHYEnableHSEValue, .true, .@"=");
            ref_out.flags.USBPHYEnable = check_ref(?USBPHYEnableList, USBPHYEnableValue, .true, .@"=");
            ref_out.flags.EnableDSI = check_ref(?EnableDSIList, EnableDSIValue, .true, .@"=");
            ref_out.flags.USBPHYEnablePll = check_ref(?USBPHYEnablePllList, USBPHYEnablePllValue, .true, .@"=");
