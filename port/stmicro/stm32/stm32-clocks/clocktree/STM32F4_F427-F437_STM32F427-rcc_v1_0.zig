...
pub fn ClockTree(comptime mcu_data: std.StaticStringMap(void)) type {
    return struct {
        ...
        pub const Flag_Output = struct {
            ...
            SAIAEnable: bool = false,
            SAIBEnable: bool = false,
            ...
            PLLI2SCLKSAIA_BEnable: bool = false,
            EnableExtClockForSAIA: bool = false,
            EnableExtClockForSAIB: bool = false,
        };
        ...
        pub fn get_clocks(config: Config) anyerror!Tree_Output {
            ...
            //POST CLOCK REF VcooutputI2SQ VALUE
            _ = blk: {
            ...
            //POST CLOCK REF VCOSAIOutputFreq_ValueR VALUE
            _ = blk: {
                if (config.flags.LTDCUsed_ForRCC) {
                    PLLLCDCLK.limit = .{
                        .min = null,
                ...
                PLLLCDCLK.value = 192000000;
                break :blk null;
            };

            //POST CLOCK REF VCOSAIOutputFreq_ValueQ VALUE
            _ = blk: {
            ...
            const PWR_Regulator_Voltage_ScaleValue: ?PWR_Regulator_Voltage_ScaleList = blk: {
                if (((check_ref(?f32, AHBOutput.get_as_ref(), 168000000, .@">")))) {
                    ...
                    break :blk item;
                } else if (((check_ref(@TypeOf(config.extra.PWREXT_OverDrive), config.extra.PWREXT_OverDrive, .PWREXT_OverDrive_DESACTIVATED, .@"=")) and ((check_ref(?f32, AHBOutput.get_as_ref(), 144000000, .@">")) or (check_ref(?f32, AHBOutput.get_as_ref(), 144000000, .@"="))))) {
                    const item: PWR_Regulator_Voltage_ScaleList = .PWR_REGULATOR_VOLTAGE_SCALE1;
                    const conf_item = config.extra.PWR_Regulator_Voltage_Scale;
                    if (conf_item) |i| {
                        if (item != i) {
            ...
            const PWREXT_OverDriveValue: ?PWREXT_OverDriveList = blk: {
                if ((check_ref(?f32, AHBOutput.get_as_ref(), 168000000, .@">")) or (check_ref(?f32, APB1Output.get_as_ref(), 42000000, .@">")) or (check_ref(?f32, APB2Output.get_as_ref(), 84000000, .@">"))) {
            ...
            ref_out.flags.SAIAEnable = check_ref(?SAIAEnableList, SAIAEnableValue, .true, .@"=");
            ref_out.flags.SAIBEnable = check_ref(?SAIBEnableList, SAIBEnableValue, .true, .@"=");
            ...
            ref_out.flags.PLLI2SCLKSAIA_BEnable = check_ref(?PLLI2SCLKSAIA_BEnableList, PLLI2SCLKSAIA_BEnableValue, .true, .@"=");
            ref_out.flags.EnableExtClockForSAIA = check_ref(?EnableExtClockForSAIAList, EnableExtClockForSAIAValue, .true, .@"=");
            ref_out.flags.EnableExtClockForSAIB = check_ref(?EnableExtClockForSAIBList, EnableExtClockForSAIBValue, .true, .@"=");
            return Tree_Output{
                .clock = out,
                .config = ref_out,
            };
