    ...
    return struct {
        ...
        pub fn get_clocks(config: Config) anyerror!Tree_Output {
            ...
            const EnbaleCSSValue: ?EnbaleCSSList = blk: {
                if (((check_MCU("PLLSourceHSE") and SysSourcePLL) or (check_ref(@TypeOf(SYSCLKSourceValue), SYSCLKSourceValue, .RCC_SYSCLKSOURCE_HSE, .@"="))) and (config.flags.HSEByPassPwr or config.flags.HSEOscillator)) {
            ...
            //POST CLOCK REF Cortex2Freq_Value VALUE
            _ = blk: {
            ...
            const FLatencyValue: ?FLatencyList = blk: {
                if ((scale1 and ((check_ref(?f32, AHB3Output.get_as_ref(), 18000000, .@"<")) or (check_ref(?f32, AHB3Output.get_as_ref(), 18000000, .@"=")))) or (scale2 and ((check_ref(?f32, AHB3Output.get_as_ref(), 6000000, .@"<")) or (check_ref(?f32, AHB3Output.get_as_ref(), 6000000, .@"="))))) {
                ...
                } else if ((scale1 and ((check_ref(?f32, AHB3Output.get_as_ref(), 36000000, .@"<")) or (check_ref(?f32, AHB3Output.get_as_ref(), 36000000, .@"=")))) or (scale2 and ((check_ref(?f32, AHB3Output.get_as_ref(), 12000000, .@"<")) or (check_ref(?f32, AHB3Output.get_as_ref(), 12000000, .@"="))))) {
                    const conf_item = config.extra.FLatency;
                    if (conf_item) |item| {
                        switch (item) {
                            .FLASH_LATENCY_1 => {},
                ...
                } else if ((scale1 and ((check_ref(?f32, AHB3Output.get_as_ref(), 48000000, .@"<")) or (check_ref(?f32, AHB3Output.get_as_ref(), 48000000, .@"=")))) or (scale2 and ((check_ref(?f32, AHB3Output.get_as_ref(), 16000000, .@"<")) or (check_ref(?f32, AHB3Output.get_as_ref(), 16000000, .@"="))))) {
            ...
            const PWR_Regulator_Voltage_ScaleValue: ?PWR_Regulator_Voltage_ScaleList = blk: {
                if (SysSourceMSI and ((check_ref(?f32, SysCLKOutput.get_as_ref(), 16000000, .@"<")) or (check_ref(?f32, SysCLKOutput.get_as_ref(), 16000000, .@"=")))) {
                    ...
                    break :blk item;
                } else if ((SysSourceHSI or SysSourceHSE) and ((check_ref(?f32, SysCLKOutput.get_as_ref(), 16000000, .@"<")) or (check_ref(?f32, SysCLKOutput.get_as_ref(), 16000000, .@"=")))) {
                    const conf_item = config.extra.PWR_Regulator_Voltage_Scale;
                    if (conf_item) |item| {
                        switch (item) {
                            .PWR_REGULATOR_VOLTAGE_SCALE2 => scale2 = true,
                ...
                } else if ((check_ref(?f32, AHB3Output.get_as_ref(), 16000000, .@"<")) or (check_ref(?f32, AHB3Output.get_as_ref(), 16000000, .@"="))) {
                    const conf_item = config.extra.PWR_Regulator_Voltage_Scale;
                    if (conf_item) |item| {
                        switch (item) {
                            .PWR_REGULATOR_VOLTAGE_SCALE2 => scale2 = true,
            ...
            ref_out.flags.HSEByPassPwr = config.flags.HSEByPassPwr;
