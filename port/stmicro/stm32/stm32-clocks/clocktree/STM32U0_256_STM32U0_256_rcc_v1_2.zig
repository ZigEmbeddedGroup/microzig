...
pub fn ClockTree(comptime mcu_data: std.StaticStringMap(void)) type {
    return struct {
        ...
        pub fn get_clocks(config: Config) anyerror!Tree_Output {
            ...
            const HSE_VALUEValue: ?f32 = if (config.HSE_VALUE) |i| i else 4000000;
            ...
            const EnbaleCSSValue: ?EnbaleCSSList = blk: {
                if (((check_MCU("PLLSourceHSE") and SysSourcePLL) or SysSourceHSE) and (config.flags.HSEOscillator or config.flags.HSEByPass)) {
            ...
            _ = blk: {
                if ((config.flags.HSEByPass or config.flags.HSEOscillator) and scale1) {
                    ...
                    HSEOSC.value = config_val orelse 4000000;
            ...
            const MSIRC_clk_value = MSIClockRangeValValue orelse return comptime_fail_or_error(error.InvalidClockValue,
                \\Error on Clock {s} | expr: {s} diagnostic: {s}
                \\Clock is active but the reference value {s} is null
                \\note: check the flags and configurations associated with this clock
                \\note2: actually I'm not sure if null values and clock nodes should be treated as errors.
            ...
            if (check_ref(@TypeOf(LPUART3EnableValue), LPUART3EnableValue, .true, .@"=")) {
                const LPUART3Mult_clk_value = LPUART3CLockSelectionValue orelse return comptime_fail_or_error(error.InvalidClockValue,
                    \\Error on Clock {s} | expr: {s} diagnostic: {s}
                    \\Clock is active but the reference value {s} is null
                    \\note: check the flags and configurations associated with this clock
                    \\note2: actually I'm not sure if null values and clock nodes should be treated as errors.
            ...
            const FLatencyValue: ?FLatencyList = blk: {
                if (scale2 and ((check_ref(?f32, AHBOutput.get_as_ref(), 8000000, .@"<")) or (check_ref(?f32, AHBOutput.get_as_ref(), 8000000, .@"=")))) {
                    ...
                    break :blk item;
                } else if (scale2 and ((check_ref(?f32, AHBOutput.get_as_ref(), 18000000, .@"<")) or (check_ref(?f32, AHBOutput.get_as_ref(), 18000000, .@"=")))) {
                    ...
                    break :blk item;
                } else if (scale1 and ((check_ref(?f32, AHBOutput.get_as_ref(), 56000000, .@"<")) or (check_ref(?f32, AHBOutput.get_as_ref(), 56000000, .@"=")))) {
                    const item: FLatencyList = .FLASH_LATENCY_2;
                    const conf_item = config.extra.FLatency;
                    if (conf_item) |i| {
                        if (item != i) {
            ...
            ref_out.flags.LPUART3_Used = config.flags.LPUART3_Used;
            ref_out.flags.LPUART2_Used = config.flags.LPUART2_Used;
            ...
            ref_out.flags.LPUART3Enable = check_ref(?LPUART3EnableList, LPUART3EnableValue, .true, .@"=");
