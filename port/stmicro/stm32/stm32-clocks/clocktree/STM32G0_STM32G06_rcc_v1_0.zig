    ...
    return struct {
        ...
        pub fn get_clocks(config: Config) anyerror!Tree_Output {
            ...
            const FLatencyValue: ?FLatencyList = blk: {
                if (scale1 and ((check_ref(?f32, AHBOutput.get_as_ref(), 24000000, .@"<")) or (check_ref(?f32, AHBOutput.get_as_ref(), 24000000, .@"=")))) {
                    ...
                    break :blk item;
                } else if (scale2 and ((check_ref(?f32, AHBOutput.get_as_ref(), 8000000, .@"<")) or (check_ref(?f32, AHBOutput.get_as_ref(), 8000000, .@"=")))) {
                    const item: FLatencyList = .FLASH_LATENCY_0;
                    const conf_item = config.extra.FLatency;
                    if (conf_item) |i| {
                        if (item != i) {
            ...
            ref_out.flags.TIM1Used_ForRCC = config.flags.TIM1Used_ForRCC;
            ...
            ref_out.flags.USART2Enable = config.flags.USART2Enable;
            ...
            ref_out.flags.TIM15Enable = config.flags.TIM15Enable;
