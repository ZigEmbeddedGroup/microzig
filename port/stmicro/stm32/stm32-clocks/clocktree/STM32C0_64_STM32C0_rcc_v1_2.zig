    ...
    return struct {
        ...
        pub fn get_clocks(config: Config) anyerror!Tree_Output {
            ...
            const FDCAN1CLockSelectionValue: ?FDCAN1CLockSelectionList = blk: {
                ...
                break :blk conf_item orelse {
                    FDCAN1SourcePCLK1 = true;
                    break :blk .RCC_FDCAN1CLKSOURCE_PCLK1;
                };
            };
            ...
            const EnbaleCSSValue: ?EnbaleCSSList = blk: {
                if (SysSourceHSE and (config.flags.HSEOscillator or config.flags.HSEByPass)) {
            ...
            if (check_MCU("FDCAN1_Exist")) {
                if (check_ref(@TypeOf(FDCAN1EnableValue), FDCAN1EnableValue, .true, .@"=")) {
                    const FDCAN1Mult_clk_value = FDCAN1CLockSelectionValue orelse return comptime_fail_or_error(error.InvalidClockValue,
                        \\Error on Clock {s} | expr: {s} diagnostic: {s}
                        \\Clock is active but the reference value {s} is null
                        \\note: check the flags and configurations associated with this clock
                        \\note2: actually I'm not sure if null values and clock nodes should be treated as errors.
            ...
            //POST CLOCK REF APBTimFreq_Value VALUE
            _ = blk: {
                TimPrescOut1.limit = .{
                    .min = null,
                    .max = 4.8e7,
            ...
            ref_out.flags.OSC_Enabled = config.flags.OSC_Enabled;
            ref_out.flags.LSEByPass = config.flags.LSEByPass;
            ref_out.flags.LSEOscillator = config.flags.LSEOscillator;
            ref_out.flags.OSC32_Enabled = config.flags.OSC32_Enabled;
            ...
            ref_out.flags.FDCAN1Enable = check_ref(?FDCAN1EnableList, FDCAN1EnableValue, .true, .@"=");
