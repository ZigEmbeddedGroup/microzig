...
pub fn ClockTree(comptime mcu_data: std.StaticStringMap(void)) type {
    return struct {
        ...
        pub const Config_Output = struct {
            ...
            HSI14_VALUE: ?f32 = null, //from RCC Clock Config
        ...
        pub fn get_clocks(config: Config) anyerror!Tree_Output {
            ...
            var HSI14CalibrationValueValue: ?f32 = if (config.extra.HSI14CalibrationValue) |i| @as(f32, @floatFromInt(i)) else 16;
            ...
            if ((!check_MCU("DIE448") and !check_MCU("DIE445"))) {
                if (check_ref(@TypeOf(EnableHSEValue), EnableHSEValue, .true, .@"=")) {
                    const HSEPLLsourceDevisor_clk_value = PLLDividerValue orelse return comptime_fail_or_error(error.InvalidClockValue,
                        \\Error on Clock {s} | expr: {s} diagnostic: {s}
                        \\Clock is active but the reference value {s} is null
                        \\note: check the flags and configurations associated with this clock
                        \\note2: actually I'm not sure if null values and clock nodes should be treated as errors.
            ...
            //POST CLOCK REF USBOutputFreqValue VALUE
            _ = blk: {
                USBOutput.limit = .{
                    .min = 4.788e7,
                    .max = 4.812e7,
            ...
            //POST CLOCK REF HSI14CalibrationValue VALUE
            _ = blk: {
                if (check_ref(@TypeOf(HSI14UsedValue), HSI14UsedValue, 1, .@"=")) {
                    ...
                    HSI14CalibrationValueValue = if (config_val) |i| @as(f32, @floatFromInt(i)) else 16;
            ...
            ref_out.HSI14_VALUE = HSI14_VALUEValue;
            ...
            ref_out.flags.HSI14Used = check_ref(?f32, HSI14UsedValue, 1, .@"=");
