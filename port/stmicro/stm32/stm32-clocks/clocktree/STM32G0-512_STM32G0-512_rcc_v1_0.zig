...
pub fn ClockTree(comptime mcu_data: std.StaticStringMap(void)) type {
    return struct {
        ...
        pub const Flag_Output = struct {
            ...
            ADCPLLEnable: bool = false,
            I2S1PLLEnable: bool = false,
            I2S2PLLEnable: bool = false,
            MCOPLLEnable: bool = false,
            MCO2PLLEnable: bool = false,
            RNGPLLEnable: bool = false,
            TIM1PLLEnable: bool = false,
            TIM15PLLEnable: bool = false,
            FDCANPllEnable: bool = false,
            USBPllEnable: bool = false,
            PLLUsed: bool = false,
            HSIUsed: bool = false,
            LSEUsed: bool = false,
        ...
        pub fn get_clocks(config: Config) anyerror!Tree_Output {
            ...
            if (!check_MCU("STM32G0x0_Value_line")) {
                if (check_ref(@TypeOf(LPUART2EnableValue), LPUART2EnableValue, .true, .@"=")) {
                    const LPUART2Mult_clk_value = LPUART2CLockSelectionValue orelse return comptime_fail_or_error(error.InvalidClockValue,
                        \\Error on Clock {s} | expr: {s} diagnostic: {s}
                        \\Clock is active but the reference value {s} is null
                        \\note: check the flags and configurations associated with this clock
                        \\note2: actually I'm not sure if null values and clock nodes should be treated as errors.
            ...
            const SYSCLKFreq_VALUE1Value: ?f32 = blk: {
                const min: ?f32 = null;
                const max: ?f32 = null;
                const val: ?f32 = try math_op(?f32, SysCLKOutput.get_as_ref(), 1000000, .@"/", "SYSCLKFreq_VALUE1", "SysCLKOutput", "1000000");
            ...
            const SYSCLKFreq_VALUE2Value: ?f32 = blk: {
                const min: ?f32 = null;
                const max: ?f32 = null;
                const val: ?f32 = try math_op(?f32, SysCLKOutput.get_as_ref(), 1000000, .@"/", "SYSCLKFreq_VALUE2", "SysCLKOutput", "1000000");
            ...
            if (!check_MCU("STM32G0x0_Value_line")) {
                if (check_ref(@TypeOf(TIM15EnableValue), TIM15EnableValue, .true, .@"=")) {
                    const TIM15Mult_clk_value = TIM15CLockSelectionValue orelse return comptime_fail_or_error(error.InvalidClockValue,
                        \\Error on Clock {s} | expr: {s} diagnostic: {s}
                        \\Clock is active but the reference value {s} is null
                        \\note: check the flags and configurations associated with this clock
                        \\note2: actually I'm not sure if null values and clock nodes should be treated as errors.
            ...
            //POST CLOCK REF TIM1Freq_Value VALUE
            _ = blk: {
                if (!check_MCU("STM32G0x0_Value_line")) {
                    TIM1output.limit = .{
                        .min = null,
            ...
            //POST CLOCK REF TIM15Freq_Value VALUE
            _ = blk: {
                if (!check_MCU("STM32G0x0_Value_line")) {
                    TIM15output.value = 4000000;
                    break :blk null;
                }
                const min: ?f32 = null;
                const max: ?f32 = null;
            ...
            //POST CLOCK REF APBFreq_Value VALUE
            _ = blk: {
                APBOutput.limit = .{
                    .min = null,
                    .max = 6.4e7,
            ...
            const FLatencyValue: ?FLatencyList = blk: {
                if (((check_ref(?f32, AHBOutput.get_as_ref(), 8000000, .@"<")) or (check_ref(?f32, AHBOutput.get_as_ref(), 8000000, .@"=")))) {
                    ...
                    break :blk item;
                } else if (scale1 and ((check_ref(?f32, AHBOutput.get_as_ref(), 24000000, .@"<")) or (check_ref(?f32, AHBOutput.get_as_ref(), 24000000, .@"=")))) {
                    const item: FLatencyList = .FLASH_LATENCY_0;
                    const conf_item = config.extra.FLatency;
                    if (conf_item) |i| {
                        if (item != i) {
                    ...
                    break :blk item;
                } else if (scale1 and ((check_ref(?f32, AHBOutput.get_as_ref(), 48000000, .@"<")) or (check_ref(?f32, AHBOutput.get_as_ref(), 48000000, .@"=")))) {
                    const item: FLatencyList = .FLASH_LATENCY_1;
                    const conf_item = config.extra.FLatency;
                    if (conf_item) |i| {
                        if (item != i) {
            ...
            const PWR_Regulator_Voltage_ScaleValue: ?PWR_Regulator_Voltage_ScaleList = blk: {
                if (((check_ref(?f32, AHBOutput.get_as_ref(), 16000000, .@"<")) or (check_ref(?f32, AHBOutput.get_as_ref(), 16000000, .@"=")))) {
            ...
            ref_out.flags.USB_DRD_FSUsed_ForRCC = config.flags.USB_DRD_FSUsed_ForRCC;
            ...
            ref_out.flags.LPUART1Used_ForRCC = config.flags.LPUART1Used_ForRCC;
            ...
            ref_out.flags.LPUART2Used_ForRCC = config.flags.LPUART2Used_ForRCC;
            ...
            ref_out.flags.EnableRTCWKUP = check_ref(?EnableRTCWKUPList, EnableRTCWKUPValue, .true, .@"=");
            ...
            ref_out.flags.I2S1Enable = check_ref(?I2S1EnableList, I2S1EnableValue, .true, .@"=");
            ...
            ref_out.flags.LPUART2Enable = check_ref(?LPUART2EnableList, LPUART2EnableValue, .true, .@"=");
            ref_out.flags.EnableCEC = check_ref(?EnableCECList, EnableCECValue, .true, .@"=");
            ...
            ref_out.flags.ADCPLLEnable = check_ref(?ADCPLLEnableList, ADCPLLEnableValue, .true, .@"=");
            ref_out.flags.I2S1PLLEnable = check_ref(?I2S1PLLEnableList, I2S1PLLEnableValue, .true, .@"=");
            ref_out.flags.I2S2PLLEnable = check_ref(?I2S2PLLEnableList, I2S2PLLEnableValue, .true, .@"=");
            ref_out.flags.MCOPLLEnable = check_ref(?MCOPLLEnableList, MCOPLLEnableValue, .true, .@"=");
            ref_out.flags.MCO2PLLEnable = check_ref(?MCO2PLLEnableList, MCO2PLLEnableValue, .true, .@"=");
            ref_out.flags.RNGPLLEnable = check_ref(?RNGPLLEnableList, RNGPLLEnableValue, .true, .@"=");
            ref_out.flags.TIM1PLLEnable = check_ref(?TIM1PLLEnableList, TIM1PLLEnableValue, .true, .@"=");
            ref_out.flags.TIM15PLLEnable = check_ref(?TIM15PLLEnableList, TIM15PLLEnableValue, .true, .@"=");
            ref_out.flags.FDCANPllEnable = check_ref(?FDCANPllEnableList, FDCANPllEnableValue, .true, .@"=");
            ref_out.flags.USBPllEnable = check_ref(?USBPllEnableList, USBPllEnableValue, .true, .@"=");
