...
pub fn clocks_enable_set(mask: PeripheralMask) void {
    var current_mask: u64 = @bitCast(@as(u64, SYSTEM.PERIP_CLK_EN0.read_raw()) | ((@as(u64, SYSTEM.PERIP_CLK_EN1.read_raw()) << 32)));
    current_mask |= @as(u43, @bitCast(mask));
    SYSTEM.PERIP_CLK_EN0.write_raw(@truncate(current_mask));
...
pub fn peripheral_reset_set(mask: PeripheralMask) void {
    var current_mask: u64 = @as(u64, SYSTEM.PERIP_RST_EN0.read_raw()) | (@as(u64, SYSTEM.PERIP_RST_EN1.read_raw()) << 32);
    current_mask |= @as(u43, @bitCast(mask));
    SYSTEM.PERIP_RST_EN0.write_raw(@truncate(current_mask));
