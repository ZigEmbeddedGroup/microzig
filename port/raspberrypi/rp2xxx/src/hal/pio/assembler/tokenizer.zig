    ...
    return struct {
        ...
        pub const Instruction = struct {
            ...
            pub const Payload = union(enum) {
                ...
                pull: Pull,
                mov: Mov,
                movtorx: if (chip == .RP2350 or chip == .RP2350_QFN80) MovToRx else noreturn,
                movfromrx: if (chip == .RP2350 or chip == .RP2350_QFN80) MovFromRx else noreturn,
                irq: Irq,
                set: Set,
            };
...
test "tokenize.instr.wait.jmppin" {
    // JMPPIN is a valid source on RP2350 and RP2350_QFN80
    inline for (comptime .{ Chip.RP2350, .RP2350_QFN80 }) |chip| {
        const tokens = try bounded_tokenize(chip, "wait 1 jmppin 1");
        try expect_instr_wait(chip, .{
            .polarity = 1,
            .source = .jmppin,
            .num = .{ .integer = 1 },
