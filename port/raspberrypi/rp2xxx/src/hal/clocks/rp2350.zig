...
/// Nominal value of built-in low-power oscillator, very imprecise and prone to drift over time
pub const lposc_freq = 32768;

// xosc limits
comptime {
    assert(xosc_freq <= 50_000_000 and xosc_freq >= 1_000_000);
}
...
pub const Source =
    ...
    pll_sys,
    pll_usb,
    /// Undocumented setting, use at own risk!
    pll_usb_primary_ref_opcg,
    clk_sys,
    clk_ref,
    clk_usb,
    clk_adc,
    ...
    pub fn is_aux_src_for(source: Source, generator: Generator) bool {
        return switch (generator) {
            ...
            .ref => switch (source) {
                .src_rosc, .src_xosc, .src_lposc => false,
                else => true,
            },
            else => true,
        };
...
pub const Generator =
    ...
    const Impl = common.GeneratorImpl(Generator, Source, u16);
    ...
    pub fn set_div(comptime generator: Generator, comptime div: u32) void {
        comptime {
            ...
            const int_component: u16 = @intCast(div >> 16);

            switch (generator) {
                .gpout0, .gpout1, .gpout2, .gpout3, .sys => {
                    // No checks required on int component, can be full 16 bits
                },
                .ref => {
                    if (int_component > std.math.maxInt(u8)) @compileError("CLK_REF only supports integer divisors of 8 bits");
                },
                .usb, .adc => {
                    if (int_component > std.math.maxInt(u4)) @compileError(std.fmt.comptimePrint("Clock generator {any} only supports integer divisors of 4 bits", .{generator}));
                },
                .peri, .hstx => {
                    if (int_component > std.math.maxInt(u2)) @compileError(std.fmt.comptimePrint("Clock generator {any} only supports integer divisors of 2 bits", .{generator}));
                },
            }
        }
...
pub const config = struct {
    pub const Global = struct {
        pub const GeneratorConfig = common.GeneratorConfig(Source, u16);
        ...
        pub fn get_frequency(cfg: Global, source: Source) ?u32 {
            return switch (source) {
                ...
                .clk_peri => if (cfg.peri) |peri_cfg| peri_cfg.frequency() else null,
                .clk_adc => if (cfg.adc) |adc_cfg| adc_cfg.frequency() else null,
                .clk_hstx => if (cfg.hstx) |hstx_cfg| hstx_cfg.frequency() else null,
    ...
    pub const preset = struct {
        ...
        pub fn default() Global {
            return system(150_000_000, 12_000_000);
        }
        ...
        /// - CLK_SYS sourced from PLL_SYS @ 150 MHz
        ...
        pub fn system(sys_freq: u32, ref_freq: u32) Global {
            ...
            cfg.ref = .{
                ...
                .integer_divisor = calculate_integer_divisor(xosc_freq, ref_freq, u16),
            };

            // Both PLL_SYS and PLL_USB are used, fixed configs for known frequencies
            const pll_sys_cfg_150_mhz: pll.Configuration = .{
                .refdiv = 1,
                .fbdiv = 125,
                .postdiv1 = 5,
                .postdiv2 = 2,
            };
            cfg.pll_sys = pll_sys_cfg_150_mhz;
            ...
            cfg.sys = .{
                ...
                .integer_divisor = calculate_integer_divisor(150_000_000, sys_freq, u16),
            };
            ...
            // HSTX gets connected to CLK_SYS
            cfg.hstx = .{
                .input = .{
                    .source = .clk_sys,
                    .freq = 150_000_000,
...
/// Configure all the tick counter periods in microseconds per
