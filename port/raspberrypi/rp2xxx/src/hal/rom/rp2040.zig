...
const intrinsics = struct {
    fn __popcountsi2(x: u32) callconv(.c) u32 {
        const f: *const signatures.popcount32 = @alignCast(@ptrCast(rom.lookup_and_cache_function(.popcount32)));
        return f(x);
    }

    fn __clzsi2(x: u32) callconv(.c) u32 {
        const f: *const signatures.clz32 = @alignCast(@ptrCast(rom.lookup_and_cache_function(.clz32)));
        return f(x);
    }

    fn __ctzsi2(x: u32) callconv(.c) u32 {
        const f: *const signatures.ctz32 = @alignCast(@ptrCast(rom.lookup_and_cache_function(.ctz32)));
        return f(x);
    }

    fn __bitreversi2(x: u32) callconv(.c) u32 {
        const f: *const signatures.reverse32 = @alignCast(@ptrCast(rom.lookup_and_cache_function(.reverse32)));
        return f(x);
    }

    fn __aeabi_memset(dest: [*c]u8, c: u8, n: usize) callconv(.{ .arm_aapcs = .{} }) void {
        const f: *const signatures.memset = @alignCast(@ptrCast(rom.lookup_and_cache_function(.memset)));
        _ = f(dest, c, n);
    }

    fn __aeabi_memset4(dest: [*c]u32, c: u8, n: usize) callconv(.{ .arm_aapcs = .{} }) void {
        const f: *const signatures.memset4 = @alignCast(@ptrCast(rom.lookup_and_cache_function(.memset4)));
        _ = f(dest, c, n);
    }
    ...
    fn __aeabi_memcpy(dest: [*c]u8, src: [*c]const u8, n: usize) callconv(.{ .arm_aapcs = .{} }) void {
        const f: *const signatures.memcpy = @alignCast(@ptrCast(rom.lookup_and_cache_function(.memcpy)));
        _ = f(dest, src, n);
    }

    fn __aeabi_memcpy4(dest: [*c]u32, src: [*c]const u32, n: usize) callconv(.{ .arm_aapcs = .{} }) void {
        const f: *const signatures.memcpy44 = @alignCast(@ptrCast(rom.lookup_and_cache_function(.memcpy44)));
        _ = f(dest, src, n);
    }
