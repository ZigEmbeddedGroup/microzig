...
/// input frequency
pub fn GeneratorConfig(Source: type) type {
    return struct {
        input: struct {
            source: Source,
            freq: u32,
...
/// comptime.
pub fn GeneratorImpl(Generator: type, Source: type) type {
    return struct {
        ...
        pub fn apply(comptime generator: Generator, comptime config: GeneratorConfig(Source), comptime sys_frequency: u32) void {
            const input = config.input;

            // TODO: Currently leave all fractional bits 0 to only support integer divisors
            const div: u32 = @as(u32, @intCast(config.integer_divisor)) << 8;
            ...
            switch (scenario) {
                ...
                .Standard => {
                    ...
                    var delay_cycles: u32 = sys_frequency / config.frequency() + 1;
...
pub fn calculate_integer_divisor(comptime input_freq: u32, comptime output_freq: u32) u24 {

    // source frequency has to be faster because dividing will always reduce.
    comptime assert(input_freq >= output_freq);

    const fixed_divisor: u24 = @intCast(input_freq / output_freq);
