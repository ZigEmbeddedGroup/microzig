...
pub const revision: Revision = if (std.mem.eql(u8, config.board_name, "micro:bit v1"))
    .v1
else @compileError();

pub const pins = struct {
    // TODO: only microbit v1
...
// TODO: should a led matrix be a driver
...
pub const display = struct {
    ...
    pub fn render(image: [height][width]u3, duration: time.Duration) void {
        ...
        while (!done.is_reached_by(nrf.time.get_time_since_boot())) {
            for (0..matrix_height) |i| {
                ...
                for (0..matrix_width) |j| {
                    // TODO: brightness based on value
                    if (matrix[i][j] != 0) {
                        pins.display.cols[j].put(0);
                    }
                }

                microzig.hal.time.sleep_ms(2);
