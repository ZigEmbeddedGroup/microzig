...
pub const Pin = enum(u6) {
    ...
    pub fn set_direction(pin: Pin, direction: Direction) void {
        const regs = pin.get_regs();
        switch (direction) {
            .in => regs.DIRCLR.write_raw(pin.mask()),
            .out => regs.DIRSET.write_raw(pin.mask()),
        }
    }
    ...
    pub inline fn put(pin: Pin, value: u1) void {
        const regs = pin.get_regs();
        switch (value) {
            0 => regs.OUTCLR.write_raw(pin.mask()),
            1 => regs.OUTSET.write_raw(pin.mask()),
        }
    }
    ...
    pub inline fn read(pin: Pin) u1 {
        const regs = pin.get_regs();
        return @truncate(regs.IN.read_raw() >> pin.index());
    }
