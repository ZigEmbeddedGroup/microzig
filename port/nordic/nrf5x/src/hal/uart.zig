...
pub const UART = enum(u1) {
    ...
    fn set_txd(uart: UART, pin: gpio.Pin) void {
        const regs = uart.get_regs();
        switch (version) {
            .nrf5283x => regs.PSELTXD.raw.write(@intFromEnum(pin)),
    ...
    fn set_rxd(uart: UART, pin: gpio.Pin) void {
        const regs = uart.get_regs();
        switch (version) {
            .nrf5283x => regs.PSELRXD.raw.write(@intFromEnum(pin)),
    ...
    fn set_cts(uart: UART, pin: gpio.Pin) void {
        const regs = uart.get_regs();
        switch (version) {
            .nrf5283x => regs.PSELCTS.raw.write(@intFromEnum(pin)),
    ...
    fn set_rts(uart: UART, pin: gpio.Pin) void {
        const regs = uart.get_regs();
        switch (version) {
            .nrf5283x => regs.PSELRTS.raw.write(@intFromEnum(pin)),
    ...
    pub fn clear_rx_rdy_event(uart: UART) void {
        uart.get_regs().EVENTS_RXDRDY.raw.write(0);
    }
    ...
    pub fn clear_tx_rdy_event(uart: UART) void {
        uart.get_regs().EVENTS_TXDRDY.raw.write(0);
    }
    ...
    pub fn clear_rx_timeout_event(uart: UART) void {
        uart.get_regs().EVENTS_RXTO.raw.write(0);
    }
};
