...
pub const I2C = enum(u1) {
    ...
    pub fn apply(i2c: I2C, config: Config) !void {
        ...
        switch (version) {
            .nrf5283x => regs.PSELSCL.write_raw(@intFromEnum(config.scl_pin)),
        ...
        switch (version) {
            .nrf5283x => regs.PSELSDA.write_raw(@intFromEnum(config.sda_pin)),
    ...
    pub fn reset(i2c: I2C) void {
        ...
        switch (version) {
            .nrf5283x => {
                regs.PSELSCL.write_raw(0xFFFFFFFF);
                regs.PSELSDA.write_raw(0xFFFFFFFF);
            },
            .nrf52840 => {
    ...
    fn write_byte(i2c: I2C, byte: u8, deadline: mdf.time.Deadline) Error!void {
        ...
        regs.EVENTS_TXDSENT.write_raw(0);
    }
    ...
    fn read_byte(i2c: I2C, deadline: mdf.time.Deadline) Error!u8 {
        ...
        regs.EVENTS_RXDREADY.write_raw(0);
        return v;
    }
    ...
    fn check_and_clear_error(i2c: I2C) Error!void {
        ...
        if (error_generated) {
            // Clear error
            regs.EVENTS_ERROR.write_raw(0x0000000);
